...
export function cleanupReviews(){update(s=>{const active=(s.reviews||[]).filter(r=>r.status==='active');const keepers=new Set(uniqueReviews(active).map(r=>r.id));const now=Date.now();active.forEach(r=>{if(!keepers.has(r.id)){r.status='retired';r.duplicateRetiredAt=now;r.duplicateOf=reviewKey(r)}})})}
