...
const firebase = fs.readFileSync('js/firebase.js','utf8');
...
assert(firebase.includes("['appConfig', 'designOverrides']") && firebase.includes('designOverrides: backup.designOverrides || {}'), 'Firebase should persist per-user overrides with app state and global overrides in separate app config');
assert(firestoreRules.includes('allow read: if true') && firestoreRules.includes('allow write: if isOwner()'), 'global design overrides should be publicly readable and owner-writable');
assert(app.includes('designOverrideCss(publishedDesignOverrides)') && app.includes('saveGlobalDesignOverrides(next)'), 'the app should validate, apply, and owner-publish global override data at runtime');
assert(designSelectorContract.includes('GitHub remains the permanent source of truth'), 'the design contract should preserve repository review as the permanent change path');
...
assert(app.includes('publishedDesignOverrides.global') && app.includes('designOverrides.route') && app.includes('designOverrides.instance'), 'component changes should use published global and personal route/instance scopes');
