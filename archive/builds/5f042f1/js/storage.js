...
export function replace(data){if(!data||data.version!==1||!data.profile||!Array.isArray(data.results))throw Error('This is not a valid Memory Dojo v1 backup.');state={...structuredClone(defaults),...data,designOverrides:data.designOverrides&&typeof data.designOverrides==='object'&&!Array.isArray(data.designOverrides)?data.designOverrides:{}};localStorage.setItem(KEY,JSON.stringify(state));window.dispatchEvent(new CustomEvent('dojo:data'));return state}
